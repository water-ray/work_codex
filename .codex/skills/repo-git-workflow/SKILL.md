...
- 切换或创建分支时，有远程源先更新并列出远程分支列表，无远程源则列出本地分支列表；输入编号选择分支，输入文本作为分支名。切换前先以“切换分支为XXX前的自动提交”提交当前改动；直接运行脚本时默认先确认，传入 `--yes` 时跳过自动提交确认；远程源已设置且远程分支存在时优先切换或同步远程分支，未设置远程源或远程分支不存在时仅创建或切换本地分支。
- 拉取远程分支会先读取当前默认远程源的所有分支并分页列出，序号从 `1` 开始，每页显示 10 个，输入 `0` 显示下一页，输入分支序号或分支名称后拉取；远程源无效或无法访问时提示先执行“仓库：置远程源 🔗”；拉取前以“拉取远程分支REMOTE/BRANCH前的自动提交”提交本地改动；直接运行脚本时默认先确认，传入 `--yes` 时跳过自动提交确认；同名分支优先合并，合并失败或分支不同才询问是否覆盖拉取。
...
```powershell
python scripts/git/repo_tasks.py status
```
...
```powershell
python scripts/git/repo_tasks.py set-remote
```

拉取远程分支：

```powershell
python scripts/git/repo_tasks.py pull-remote
```

如需跳过拉取前自动提交确认：

```powershell
python scripts/git/repo_tasks.py pull-remote --yes
```

拉取任务会先获取当前默认远程源的真实远程分支列表，按每页 10 个显示。输入 `0` 翻页，输入整数按序号选择远程分支，输入其他文本则作为远程分支名称；直接回车使用当前本地分支名。远程源无效或无法访问时，先执行“仓库：置远程源 🔗”更新远程源。

切换或创建分支：

```powershell
python scripts/git/repo_tasks.py switch
```
...
```powershell
python scripts/git/repo_tasks.py switch --yes
```

提交当前全部改动：

```powershell
python scripts/git/repo_tasks.py commit
```
...
```powershell
python scripts/git/repo_tasks.py push
```
...
```powershell
python scripts/git/repo_tasks.py restore-node
```
...
- 拉取远程分支任务输出远程源真实分支分页列表，便于按序号选择要拉取的分支。
